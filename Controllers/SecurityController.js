...
class LogController {
  ...
  async addLogDetails(req, res) {
    ...
    try {
      ...
      const resident = await ApartmentUser.findOne({
        username: name,
  ...
  async getLogDetails(req, res) {
    ...
    try {
      ...
      if (logs.length === 0) {
        return res
          .status(200)
          .json({ error: "No logs found for this resident" });
      }
  ...
  async addGuests(req, res) {
    try {
      ...
      if (
        ...
        !no_of_guests ||
        !guest_names ||
        guest_names.length != no_of_guests
      ) {
      ...
      return res
        .status(201)
        .json({ message: "Guests added successfully", guests });
    } catch (error) {
