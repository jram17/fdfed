...
const verifyUser = async (
  ...
  try {
    const isvalid = SignInDto.safeParse({ email: username, password });
    if (!isvalid.success) {
      return done(null, false, {
        message: "Validation error in logging in the user",
      });
    }
    const { email, password: parsedpassword } = isvalid.data;
    ...
    const isValid = verifyPassword(parsedpassword, user.password_hash);
    if (!isValid) {
  ...
  new GoogleStrategy(
    ...
    async (
      ...
      try {
        ...
        if (email == undefined) {
          return cb("Email not found in the google profile", false);
        }
        const user = await db.user.findFirst({
          where: {
            email: email,
